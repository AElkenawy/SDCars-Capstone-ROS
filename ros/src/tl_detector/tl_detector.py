...
class TLDetector(object):
    ...
    def waypoints_cb(self, waypoints):
        # Store waypoints in self.waypoints object
    ...
    def image_cb(self, msg):
        ...
        # process_traffic_lights gives state and waypoint of traffic light
        ...
        used.
        '''
        # Assuring traffic light state stabiliy (when changing/noisy)
        
        # If state is changed >> reset counter, store the new state
        if self.state != state:
            self.state_count = 0
            self.state = state
        
        # If state counter greater than a threhold
        elif self.state_count >= STATE_COUNT_THRESHOLD:
            self.last_state = self.state
            # light_wp to be only published in case of Red traffic light
...
#         return self.light_classifier.get_classification(cv_image)
    ...
    def process_traffic_lights(self):
        ...
        # Using KDTree of get_closest_waypoint
        if(self.pose):
            car_wp_idx = self.get_closest_waypoint(self.pose.pose.position.x, self.pose.pose.position.y)

         # Find the closest visible traffic light (if one exists)
        
            # Start with a big value difference
        ...
        # If don't get a traffic light or unknown traffic classification
